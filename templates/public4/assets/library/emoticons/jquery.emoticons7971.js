...
;(function (factory) {
    ...
    $.emoticons = function(parameter,getApi) {
        ...
        var defaults = {
            'prefix':'widget',
            'publisherCls':'commentPopup',
            'triggerCls':'trigger',
            'activeCls':'active',
            'path':'/templates/public4/assets/library/emoticons/emoji/',
            'list':[
        ...
        $('.'+options.prefix+'-layer').remove();
        var _api = {};
        var $document = $(document);
        var $body = $('body');
        var $layer = $('<div class="'+options.prefix+'-layer">').appendTo($body);
        var $tool = $('<div class="'+options.prefix+'-tool"></div>').appendTo($layer);
        var $close = $('<a class="'+options.prefix+'-close" href="javascript:;" title="关闭"><span class="icon-close"></a>').appendTo($tool);
